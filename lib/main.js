...
function scheduleUpload() {
  ...
  Fuse({ start: ss.lastupload,
         ...
         resolve_this: self }).then(
    function _upload() {
      monitor.record(ss.storage.cookiemonster).then(function _record(aData) {
        console.log("Recording data collected...");
        console.log("Data collected:\n\n");
        console.log("The data: " + aData);
        console.log("Uploading...");
        micropilot.upload(UPLOAD_URL).then(micropilot.clear);
