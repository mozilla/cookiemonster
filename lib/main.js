 ...
 */
function parseAndLogCookies(cookies, domain, referrer, success, existingCount) {
  let parts = cookies.split("\n");
...
function onExamineResponse(event) {
  ...
  let domain = getBaseDomain(channel.URI);
  let existingCount = Services.cookiemgr.countCookiesFromHost(channel.URI);
  ...
  try {
    ...
    return parseAndLogCookies(cookies, domain, referrerDomain, true, existingCount);
