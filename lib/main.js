...
function onModifyRequest(event) {
  ...
  if (checkForPrivateChannel(channel)) {
    console.log("Discovered private channel, abort cookie examination.");
    return;
  }
...
function parseAndLogCookies(cookies, domain, referrer) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    console.log("cookie:", JSON.stringify(kv));
...
// Upload study data daily
function scheduleUpload() {
  console.log("scheduleUpload()");
  // let uploadDuration = (86400 * 1000);
  let uploadDuration = (60 * 300 * 1000);
  if (!ss.lastupload) {
  ...
  Fuse({ start: ss.lastupload,
         ...
         pulsefn: function _upload() {
           monitor.record(cookiemonster).then(function _record(aData) {
            ...
            } catch (ex) { console.log(ex); console.log(ex.stack);} // end try
           });
         },
         resolve_this: self }).then(function (){ console.log("\n\n...then...\n\n");   });
}
scheduleUpload();

function checkForPrivateChannel(aChannel) {
  console.log("checkForPrivateChannel()");
  let channel = aChannel.QueryInterface(Ci.nsIPrivateBrowsingChannel);
  console.log(channel);
  console.log(channel.isChannelPrivate);
